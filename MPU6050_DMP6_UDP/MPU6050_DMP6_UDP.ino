...
const IPAddress outIp(192, 168, 10, 17);     // remote IP to receive OSC
const unsigned int outPort = 9995;          // remote port to receive OSC
...
void mpu_loop()
{
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
    ...
    // otherwise, check for DMP data ready interrupt (this should happen frequently)
        ...
        Serial.println(F("Not Over Flow!"));
...
void loop(void)
{
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println();
    Serial.println("*** Disconnectedi from AP so rebooting ***");
    Serial.println();
    ESP.reset();
  }
  mpu_loop();
